...
from forte.browser.file_browser import FileBrowser
from forte.build.build_manager import BuildManager
from forte.config.settings import Settings
from forte.config.settings_dialog import SettingsDialog
from forte.config.themes import apply_theme_to_app, get_theme
from forte.editor.find_replace import FindReplaceDialog
from forte.editor.tab_widget import EditorTabWidget
from forte.terminal.terminal_widget import TerminalWidget

_OPEN_FILTER = (
    "Fortran Files (*.f90 *.f95 *.f03 *.f08 *.f *.for *.f77 *.ftn);;"
    "All Files (*)"
)
_SAVE_FILTER = (
...
class MainWindow(QMainWindow):
    """FORTE main application window."""

    def __init__(self):
        super().__init__()
        self.settings = Settings()
        self.setWindowTitle("FORTE — FORtran Teaching Environment")
    ...
    def _create_menus(self) -> None:
        ...
        about_action = QAction("&About FORTE", self)
    ...
    def _on_tab_changed(self, index: int) -> None:
        ...
        if editor:
            try:
                ...
                # PyQt6 raises TypeError (not RuntimeError) when the signal
                # was never connected — safe to ignore on first use of a tab.
                pass
    ...
    def open_file(self) -> None:
        path, _ = QFileDialog.getOpenFileName(
            self, "Open Fortran File", "", _OPEN_FILTER
        )
        if not path:
            return
        try:
    ...
    def save_file_as(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save Fortran File", "", _SAVE_FILTER
        )
        if path:
    ...
    def show_about(self) -> None:
        QMessageBox.about(
            ...
            "Version 0.1.0\n\n"
            "A beginner-friendly IDE for learning and teaching Fortran.\n\n"
